...
public class ReadingTrackerMain {
    ...
    /**
     * Prints out user menu in the terminal
     */
    public static void printInterface(){
        System.out.println("""
                =================================================================================================================================================================================
                Welcome to the reading tracker! Please select one of the following options:

                Manage book log:
                   1) Create new book log
                   2) View all books in book log

                Manage reading list:
                   3) Add a book to reading list
                   4) View all books in reading list
                   5) Transfer book from reading list to book log
                   6) What book should you read next?

                Request reading statistics:
                   7) Request a list of all book info
                   8) Request a list of books from book log of a certain rating
                   9) Request reading stats for a certain month
                   10) Request genre stats for a certain genre
                   11) Get a list of the books sorted from highest rated to lowest rated
                   
                File options:
                   12) Save reading tracker data to file
                   13) Load reading tracker data from file

                x) Exit program
                =================================================================================================================================================================================
                
                Enter (1-13 or x) to select an option:""");
    }
     ...
     * @param bookLog hashmap of book log
     */
    public static void viewBookLog(Map<String, BookLogItem> bookLog){
        // Get amount of books in book log
        int logCount = bookLog.size();

        if (logCount > 1){
            // Print out number of books in book log
            System.out.println("You have " + logCount + " books in your Book Log, they are: ");
            ...
            // Print out number of books in book log
            System.out.println("You have " + logCount + " book in your Book Log, it is: ");
            ...
            // Print out number of books in book log
            System.out.println("You have no books in your Book Log!");
        }

    }

    /**
     * Gets book info from user, adds book info to readingList hashmap at the key (title) and the value (arrayList of book
     * info)
     ...
     */
    public static void newReadingList(String info, Map<String, ReadingListItem> readingList){
        // split the book info into an array by commas
        String[] bookInfoList = info.split(",");
        ...
        // retrieve author
        String author = bookInfoList[AUTHOR_INDEX].strip();
        // retrieve read want amount
        int readWantAmount = Integer.parseInt(bookInfoList[READING_WANT_AMOUNT_INDEX].strip());
        // retrieve genre
        String genre = bookInfoList[GENRE_INDEX_R_LIST].strip();

        // Create new ReadingListItem
        ReadingListItem newRList = new ReadingListItem(bookTitle, author, genre, readWantAmount);

        // Place ReadingListItem in reading list hashmap with book title as key
        readingList.put(bookTitle, newRList);
    ...
    public static boolean listToLogInputCheck(String info){
        ...
        // Check if user entered any command line arguments
        if (args.length == 1){
            // If one argument is present, use it as a file name and run readFile to start program
            String fileName = args[0];
            readFile(fileName, bookLog, readingList);
        }
        else if (args.length > 1){
            // If there is more than one argument entered, print message and run program as normal
            System.out.println("Too many arguments entered, could not run program from command line, running as normal");
        }
        ...
        // While input is not "x", do the user requested action then request more input
        while (!Objects.equals(userSelection, "x")){
            //(do what input wants)
            if(Objects.equals(userSelection, "1")) {
                // get book info from user
                System.out.println("Please enter your books title, author, month finished (1-12), rating (1-5), number of pages, and" +
                        " genre (fantasy, classics, mystery, non fiction, sci-fi, thriller, romance) as comma separated values");
                Scanner bookInfo = new Scanner(System.in);
                String info = bookInfo.nextLine();

                while (!bookLogInputCheck(info)){
                    System.out.println("Please enter your books title, author, month finished as a number(Jan->1, Feb->2), rating (1-5), number of pages, and" +
                            " genre (fantasy, classics, mystery, non fiction, sci-fi, thriller, romance) as comma separated values");
                    info = bookInfo.nextLine();
                }
                // Call newBookLog function
                newBookLog(info, bookLog);

            }
            else if(Objects.equals(userSelection, "2")){
                // Call viewBookLog function
                viewBookLog(bookLog);
            }
            else if(Objects.equals(userSelection, "3")){
                // get book info from user
                System.out.println("Please enter your books title, author, how much you want to read it (1-10) and genre " +
                        "(fantasy, classics, mystery, non fiction, sci-fi, thriller, romance) as comma separated values");
                Scanner rListInfo = new Scanner(System.in);
                String info = rListInfo.nextLine();

                // Check that input was valid, if not, ask for new info
                while (!readingListInputCheck(info)){
                    System.out.println("Please enter your books title, author, how much you want to read it (1-10) and genre " +
                            "(fantasy, classics, mystery, non fiction, sci-fi, thriller, romance) as comma separated values");
                    info = rListInfo.nextLine();
                }

                // Call newReadingList function
                newReadingList(info, readingList);
            }
            else if(Objects.equals(userSelection, "4")){
                // Call viewReadingList function
                viewReadingList(readingList);
            }
            else if(Objects.equals(userSelection, "5")){
                // Get additional info necessary for book log from the user
                System.out.println("Please enter the title, month finished as a number(Jan->1, Feb->2), rating(1-5), and number of pages" +
                        " of the book you wish to move from reading list to book log as comma separated values: ");
                Scanner getInfo = new Scanner(System.in);
                String info = getInfo.nextLine();

                // Check that input was valid, if not, ask for new info
                while (!listToLogInputCheck(info)){
                    System.out.println("Please enter the title, month finished as a number(Jan->1, Feb->2), rating(1-5), and number of pages" +
                            " of the book you wish to move from reading list to book log as comma separated values: ");
                    info = getInfo.nextLine();
                }

                // Call listToLogFunction
                listToLog(info, bookLog, readingList);

            }
            else if(Objects.equals(userSelection, "6")){

                String nextRead = readNext(readingList);
                // Print result in terminal
                System.out.println("You should read " + nextRead + " next!");

            }
            else if(Objects.equals(userSelection, "8")){
                // Call viewRating function
                viewRating(bookLog);
            }
            else if(Objects.equals(userSelection, "9")){
                // Call month stats function
                monthStats(bookLog);
            }
            else if(Objects.equals(userSelection, "10")){
                // Call genre stats function
                genreStats(bookLog, readingList);
            }
            else if(Objects.equals(userSelection, "7")){
                printAllBookInfo(readingList, bookLog);
            }
            else if(Objects.equals(userSelection, "11")){

                // Run sortByRating function
                ArrayList<BookLogItem> sortedBooks = sortByRating(bookLog);

                // Print each book in sorted order, highest to lowest
                System.out.println("Your favourite book log titles listed highest to lowest are: ");
                for (int i = sortedBooks.size() - 1; i >= 0; i--){
                    System.out.println(sortedBooks.get(i));
                }
            }
            else if(Objects.equals(userSelection, "12")){
                // Get save filename from user
                System.out.println("Please enter a valid filename for the save file (warning: previous file contents " +
                        "will be overwritten!: ");
                Scanner getName = new Scanner(System.in);
                String saveFileName = getName.nextLine();

                // call saveToFile method
                saveToFile(saveFileName, bookLog, readingList);

            }
            else if(Objects.equals(userSelection, "13")){
                // Get load filename from user
                System.out.println("Please enter a valid filename for the information file: ");
                Scanner getName = new Scanner(System.in);
                String loadFileName = getName.nextLine();

                // Call readFile function
                readFile(loadFileName, bookLog, readingList);
